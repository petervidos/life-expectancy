...
def generateData(df,dfCountry,age,country,year):  
    ...
    dfData = pd.concat([dfData,pd.DataFrame(item)])
    if remainingYears>=0:
        item=generateDataItem(3,1,"Years lived",age,title=f"You have {remainingYears:,.0f} years remaining, according to your life expectancy at birth in {country}",lightness="yes")
        dfData = pd.concat([dfData,pd.DataFrame(item)])
        item=generateDataItem(3,1,"Years Remaining",remainingYears,lightness="no")
        dfData = pd.concat([dfData,pd.DataFrame(item)])
        ...
        item=generateDataItem(3,1,"Expected years",lifeExpectancy,title=f"You have live {abs(remainingYears):,.0f} years more than to your life expectancy at birth in {country}")
        dfData = pd.concat([dfData,pd.DataFrame(item)])
        item=generateDataItem(3,1,"Earned years",abs(remainingYears))
        dfData = pd.concat([dfData,pd.DataFrame(item)])
        percentageLived=abs(remainingYears)/lifeExpectancy*100
        item=generateDataItem(4,1,"Lived",100,title=f"Then, you have lived {percentageLived:,.1f} % more of your life in {country}")
        dfData = pd.concat([dfData,pd.DataFrame(item)])
        item=generateDataItem(4,1,"Earned extra time",percentageLived)
        dfData = pd.concat([dfData,pd.DataFrame(item)])
    position=None
    ...
    today = date.today()
    yearsRange=list(range(year-30, today.year,10))
    if yearsRange[-1]<today.year:
        yearsRange.append(today.year)
    ...
    for index, row in dfYears.sort_values("Year",ascending=False).iterrows():
        ...
        dfData = pd.concat([dfData,pd.DataFrame(item)])
    dfData.loc[(dfData["title"]==country),"title"]=f"This is how life expectancy in {country} has evolved in the decades before and after {year}"

    # Generate data items for life expectancy from birth year to current age + birth year
    dfYears=dfCountry[(dfCountry["Year"]>=year) & (dfCountry["Year"]<=today.year)].sort_values("Year").reset_index(drop=True)
    ...
    for index, row in dfYears.iterrows():
        ...
        if index==0:
            title=country
        yearChart=int(row["Year"])
        item=generateDataItem(7,index+1,f"{yearChart:,.0f}",row["age_0"],lightness=lightness,title=title)
        dfData = pd.concat([dfData,pd.DataFrame(item)])
...
def calculateAge(par_year):
    """Calculates age based on birth date."""
    today = date.today()
    age = today.year - par_year#birthDate.year -((today.month, today.day) <(birthDate.month, birthDate.day)) 
...
with cols[1]:
    par_year = st.number_input('Year Born (1950-2024)', min_value=1950, max_value=2024, value=1980)
...
if btnGenerate:  
    with st_lottie_spinner("https://lottie.host/e5744f08-8d31-4469-adee-3264439e2ca6/njHAHxUJBQ.json"):
        age=calculateAge(par_year)
        dfData,dfDataCountry = getLifeExpectancyData(par_gender,par_year,age,par_country)
        ...
        df = generateData(dfData,dfDataCountry,age,par_country,par_year)
                    ...
                    Config(
                        {
                            ...
                            "geometry": "rectangle",
                            # "x": "category",
                            "x": {"set": "value", "range": {"min": "0", "max": "auto"}},
                            "color": None,
                            "lightness": None,
                            "size": None,
                            "noop": None,
        ...
        # Slide 2A: You have ## years remaining
        slide02=Slide()
        ...
        title=f"Life expectancy for {par_country} in {par_year} was higher than {lowerCountries} countries"
        slide05.add_step(
            Step(       
                Config(
                    {
        ...
        title=f"This is life expectancy in {par_country} compared with he highest and lowest in {par_year}"
        story.add_slide(
            Slide(
                Step(
